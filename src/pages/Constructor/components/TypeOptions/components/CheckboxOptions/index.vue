...
<script setup>
import { BasicOptionForRadio } from 'src/formElementConstructors/Construsctors.js'
defineOptions({
  name: 'CheckboxOptionsComponent'
});
const props = defineProps({
  formObj: Object,
})
function updateOptions (operate, target) {
  try {
    if (!Array.isArray(target)) throw new Error('The target you want to be updated is not Array.')
    if (operate) {
      target.push(new BasicOptionForRadio())
